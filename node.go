...
// Node is the default implementation of NodeInterface.
// It wraps config.Config and optionally maintains a persistent SSH connection.
//
// The Node struct stores all configuration in a config.Config and tracks
...
//
//	node := ork.NewNode("server.example.com").SetPort("2222")
func (n *Node) SetPort(port string) NodeInterface {
	n.cfg.SSHPort = port
	return n
}
...
//
//	node := ork.NewNode("server.example.com").SetUser("deploy")
func (n *Node) SetUser(user string) NodeInterface {
	n.cfg.RootUser = user
	return n
}
...
//
//	node := ork.NewNode("server.example.com").SetKey("production.prv")
func (n *Node) SetKey(key string) NodeInterface {
	n.cfg.SSHKey = key
	return n
}
...
//	    SetArg("username", "alice").
//	    SetArg("shell", "/bin/bash")
func (n *Node) SetArg(key, value string) NodeInterface {
	if n.cfg.Args == nil {
...
//	}
//	node := ork.NewNode("server.example.com").SetArgs(args)
func (n *Node) SetArgs(args map[string]string) NodeInterface {
	n.cfg.Args = args
	return n
}

// Stub implementations for remaining NodeInterface methods
// These will be properly implemented in subsequent tasks (2.4, 2.5, 2.6)
...
func (n *Node) GetConfig() config.Config {
	...
	// Deep copy the Args map to prevent external modification
	if n.cfg.Args != nil {
...
func (n *Node) Playbook(name string) error {
	...
	// Execute playbook with current configuration
